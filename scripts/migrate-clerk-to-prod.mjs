...
const updates = SIMPLE.map(([t, c]) =>
  `UPDATE ${t} SET ${c} = m.new_id FROM clerk_id_map m WHERE ${t}.${c} = m.old_id;`
).join('\n')

const checks = SIMPLE.map(([t, c]) =>
  `SELECT '${t}.${c}' AS col, count(*) AS still_old FROM ${t} WHERE ${c} IN (SELECT old_id FROM clerk_id_map)`
).join('\nUNION ALL\n')
...
-- NOT destructive (pure ID rewrite), but run it INSIDE the maintenance window:
-- after prod users exist, immediately before the Vercel key swap. Re-runnable —
-- rows already remapped no longer match old_id.

BEGIN;

CREATE TEMP TABLE clerk_id_map (old_id TEXT PRIMARY KEY, new_id TEXT NOT NULL UNIQUE) ON COMMIT DROP;
INSERT INTO clerk_id_map (old_id, new_id) VALUES
${values};
...
FROM (
  SELECT c2.id,
         (SELECT string_agg(COALESCE(m.new_id, t.part), '|' ORDER BY COALESCE(m.new_id, t.part))
          ...
          LEFT JOIN clerk_id_map m ON m.old_id = t.part) AS new_key
  FROM conversations c2
  WHERE c2.direct_key IS NOT NULL
) sub
WHERE c.id = sub.id AND c.direct_key IS DISTINCT FROM sub.new_key;

-- Verification: every row should be 0 before COMMIT.
${checks};
