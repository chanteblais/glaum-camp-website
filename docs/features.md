...
- **API:** `GET/POST/DELETE /api/shift-signups` — per-occurrence slots with per-night counts (many-to-many ∪ legacy single, deduped), the member's owed requirements (derived group/role merged with universal tasks, max hours per type), sign-up (`occurrence_date` validated against the shift's real nights; admin notified), cancel (`occurrence_date` query param; also clears the legacy `camp_signups.schedule_event_id` so hours never double-count).
