...
- **Shift Hours** — real hours from the shift schedule (`lib/shift-hours-overview.ts` `getShiftHoursOverview`): totals (total scheduled shift hours / hours filled / empty-shift count) + a per-shift-type breakdown card coloured to match the schedule's shift palette (`shiftHue`) with scheduled hours, filled hours, signups, and empty-shift count. Filled = each signup (`member_shift_signups` ∪ legacy single, deduped) × its slot's duration (`shiftDurationHours`). **Recurring shifts count once per occurrence** (their `recurrence_days`, or every day of the configured event range — same rules as the schedule calendar), and a series signup fills every occurrence; note member attunement (`lib/shift-attunement.ts`) still credits a recurring shift's hours once — known divergence. Deliberately **no completion %/bars**: an uncapped shift has no defined "full", so the display sticks to counts (the shared-resources lesson — item counts, not unit-%). Events whose shift type was deleted bucket as "Untyped". Replaced the flat 3h-per-member estimate (2026-07-03)
