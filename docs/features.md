...
- **Leads (migration `048`):** members holding a shift can **offer to lead** it (secondary link in the "Your Shifts" card; same `POST /api/shift-signups` with `role: 'member' | 'lead'`). The role lives on the `member_shift_signups` row — it disappears with the signup, co-leads are just multiple lead rows, and it's **display-only** (no permissions attach). Picker cards show "✦ Led by …"; admins promote/demote from the member's shifts card on `/admin/[id]` (`set_shift_role` on `PATCH /api/admin/signups/[userId]`). Lead-up gatherings share the same idiom (`lead_up_event_rsvps.role`; "Offer to lead ✦" under the RSVP button; a member lead supersedes the free-text `host` in the byline). General `event_rsvps` deliberately excluded.
