...
Deliberately not chat (no threads, no replies) and deliberately **not an audit log**: every post passes "would the average member care?" and is written editorially — a headline plus an optional momentum line ("Only one more to go!"), all copy centralized in `lib/radio.ts`'s post builders. Timestamps whisper (tiny corner clock); content leads. Day-grouped (Today / Yesterday / date), newest first, latest 60; actor avatars joined fresh at read time.

Each **kind** wears its own card: `broadcast` 📢 (gold wash), `welcome` 👋 (purple tint + face), `contribution` ✨ (gold momentum detail), `achievement` 🏅 (ringed medal disc, engraved italic detail), `milestone` 🎉 (centered celebration band), `voice` ✦ (quiet italic + "— Name" attribution). The variety gives the feed rhythm.

**Members write to it too** — Radio is an interactive heartbeat: a one-line composer under the strip ("Put a moment on the air…", `POST /api/radio`, ≤200 chars → a `voice` post). No replies; a voice may drift down the stream and that's fine.
...
**Tabs** (sticky `AdminNav` on every admin page, incl. nested ones): **Overview** (`/admin/overview`, read-only monitoring) · **Members** (`/admin`, operate on people and live activity) · **Program** (`/admin/program`, the schedule and the runway to the event) · **Configure** (`/admin/configure`, standalone definitional structures). Category metadata + anchor ids live in `app/admin/admin-sections.ts`; `AdminNav` renders jump-links to each category and the days-to-camp runway strip.
...
| Radio | `RadioManager` | The curated feed's console (spec: [radio.md](radio.md)): a composer (message ≤280 + emoji + optional **"Also alert members"** bell/email), the **Automatic broadcasts** toggles (`config_radio` — welcomes / contributions / achievements / milestones / member voices, all default on), and **Recently on the air** — the latest 40 posts of any kind with two-tap Remove (curation: Radio is opinionated) |
