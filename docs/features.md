...
- **Shift Hours** — real hours from the shift schedule (`lib/shift-hours-overview.ts` `getShiftHoursOverview`): totals (total scheduled shift hours / hours **promised** / hours filled / empty-shift count) + a per-shift-type breakdown card coloured to match the schedule's shift palette (`shiftHue`) with scheduled hours, promised hours, filled hours, signups, and empty-shift count. **Promised** = the demand side — what approved members collectively owe by requirement: universal typed `shift` attunement tasks (everyone) merged with group/role `required_shift_type_id`/`required_shift_hours` (max per type per member — attunement's own rule); a type with promised hours but nothing scheduled still shows. Filled = each signup (`member_shift_signups` ∪ legacy single, deduped) × its slot's duration (`shiftDurationHours`). **Recurring shift events count once per occurrence** (their `recurrence_days`, or every day of the configured event range — same rules as the schedule calendar), but **attendance is per-night**: one signup fills one occurrence's hours (same count-once credit as member attunement). The signup row carries no date, so a recurring series' empty-shift count is occurrences − signups (which night someone works is unrecorded — see the per-night-signups model gap). Deliberately **no completion %/bars**: an uncapped shift has no defined "full", so the display sticks to counts (the shared-resources lesson — item counts, not unit-%). Events whose shift type was deleted bucket as "Untyped". Replaced the flat 3h-per-member estimate (2026-07-03)
