...
- **Profile Details** (`ProfileDetails.tsx`, client) — an editable card for the member's registry-defined detail fields (bio, quote, and any admin-added custom fields), reading/writing `member_profiles.values` via `/api/profile/fields`. Only shows fields that are **Visible** (`public`) or member-editable; admin-only fields (not Visible) never appear here — they surface on `/admin/[id]`. Renders nothing until the registry has such fields. The fields are defined in the Profile Field registry (Admin → Configure → **Profile Fields**, `ProfileFieldsManager`); see [profile-architecture.md](profile-architecture.md).
  - **Gentle completion nudge:** empty fields flagged for catch-up surface as a soft purple callout at the top of the card ("*One more detail / A few details would round out your profile — all optional…*") with each gap field marked **Optional** (dismissible via **"Not now"**) or **Required**, editable inline; the gap clears the moment it's filled. An empty **About** (Bio) is nudged **by default** — no admin setup — so a bare profile always gets one gentle prompt. This is the *only* surface for the nudge (kept off the dashboard by design, to stay non-intrusive). See [profile-architecture.md](profile-architecture.md) → Phase 4.5.
