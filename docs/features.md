...
The dedicated **Participate** page (`/participate`) hosts `SignupSection` (role + shift) and, below it, the **Your Groups** group opt-in (`GroupCommitments` — see [Groups](#groups)). The Commitments card's "Manage commitments →" link points here.
