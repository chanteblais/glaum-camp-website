...
- **Shift Hours** — real hours from the shift schedule (`lib/shift-hours-overview.ts` `getShiftHoursOverview`): totals (hours filled / still unfilled / slot count) + a per-shift-type breakdown card with a fill bar, coloured to match the schedule's shift palette (`shiftHue`). Filled = each signup (`member_shift_signups` ∪ legacy single, deduped) × its slot's duration (`shiftDurationHours`) — same math as member attunement; needed = duration × capacity, an uncapped slot counting as one person (footnoted). Events whose shift type was deleted bucket as "Untyped". Replaced the flat 3h-per-member estimate (2026-07-03)
