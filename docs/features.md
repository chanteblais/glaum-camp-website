...
- **Per field:** enable/disable, label, `key` (internal identity), description, type (Text / Long text / Number / Yes-No / Single-select / Multi-select / Date, with options for the selects), reorder, delete. Multi-selects can flag options as **stand-alone** ("Stands alone" chips under the options input, `exclusiveOptions`) — picking one clears every other selection in the Profile Details editor, shown after an "or" divider; shared logic in `lib/multi-select.ts`. (An application question bound to the field carries its own options, so its stand-alones are flagged separately in the builder via the per-option "or" pill.) Plus capability toggles:
