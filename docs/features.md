...
| `resources` | Bring Something | Unmet shared-resource needs (name + "N more needed" pill, top 6 + `+N more`; `getUnmetResourceNeeds` in `lib/resources.ts`, offers excluded) with a "Claim what you can bring →" footer to `/participate#bring`. Demand-driven: hidden once everything's covered |
