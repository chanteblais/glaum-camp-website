...
Camp dues — the shared money members chip in. **This year dues are collected by email / e-transfer** (payment happens outside the app). Because there's no automatic reconciliation, dues move through three states: **owed → awaiting review** (the member self-reports paying) **→ paid** (an admin confirms). A Stripe integration is a deliberate **future TODO** — the schema is already shaped for it (a webhook would set the same `members.dues_paid_at`). State columns: `members.dues_paid_at` / `dues_paid_by` / `dues_note` (migration `065`) + `dues_reported_at` (migration `066`), and `volunteers.dues_paid_at` / … (migration `067`); settings + payment info: `page_content.config_dues` (`lib/dues.ts`, `parseDuesConfig`).
