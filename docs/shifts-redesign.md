...
## Per-night occurrences (2026-07-03, migration `063`)
...
- **Member picker is a calendar** (Chante prefers picking by day): day columns derived from the shifts' real dates, cards coloured by type, owed-hours chips above, type legend below; one row on desktop (no scroll/wrap), stacked day sections on mobile; panel breaks out wider than the `/participate` text column on desktop. Native `confirm()` replaced with a styled `ShiftConfirmModal` (used by picker cards and the Your Shifts card).
