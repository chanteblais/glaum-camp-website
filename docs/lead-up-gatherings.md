...
**Leads (added 2026-07-02, migrations `048`+`049`):** the organizer decides per gathering whether it **has** a lead role ("Has a lead ✦" toggle in the admin modal → `lead_up_events.needs_lead`, default off). On lead-enabled gatherings, an RSVP'd member can **offer to lead** ("Offer to lead ✦" under the Going button; the API rejects lead offers on non-lead gatherings). The role lives on the `lead_up_event_rsvps` row (`role: 'member' | 'lead'`, same idiom as `member_shift_signups`), so it vanishes with the RSVP; co-leads allowed; display-only. A member lead supersedes the free-text `host` in the byline ("led by X" instead of "with {host}"). Admin manager shows "✦ Led by …" per gathering.
