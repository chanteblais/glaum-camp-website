...
**Leads (added 2026-07-02, migration `048`):** an RSVP'd member can **offer to lead** a gathering ("Offer to lead ✦" under the Going button). The role lives on the `lead_up_event_rsvps` row (`role: 'member' | 'lead'`, same idiom as `member_shift_signups`), so it vanishes with the RSVP; co-leads allowed; display-only. A member lead supersedes the free-text `host` in the byline ("led by X" instead of "with {host}"). Admin manager shows "✦ Led by …" per gathering.
