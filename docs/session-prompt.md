...
Admin-configurable groups (e.g. Setup/Teardown/Decor) in **Admin → Groups** (`GroupsManager.tsx`): create/reorder, assign members, view rosters, **upload a per-group icon image** (`groups.icon_image`, migrations `034`+`035`, public `group-badges` bucket). Tables `groups` + `group_members` (migration `030`). Members are admin-assigned; applicants can opt in via a `group_select` form field, and approved members can self-manage the same offered groups via the **Your Contributions** section on `/signup` (`GroupCommitments` → `/api/groups/membership`). Member-facing "contributions" (the profile **Active Commitments** card — groups + shift rows, each using the group's emoji `icon` / uploaded `icon_image` / `✦`; the attunement task; personal-schedule filtering; overview/registry) read group membership via `lib/groups.ts`. Full spec in `docs/features.md` → Groups.
