...
## Conventions & gotchas
- **Branching:** `main` = deployable (push = prod deploy). Non-trivial work goes on a short-lived `type/slug` branch (`feat/` `fix/` `ux/` `docs/` `chore/`), verified (`tsc` + local click-through), merged `--no-ff`, branch deleted. Tiny tweaks may go straight to `main`. Claude never pushes. Full rules: `docs/branching.md`.
