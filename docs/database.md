...
- `catering_kitchen_state` — JSON blob for the **kitchen board** (`public/kitchen.html`), the caterer's shared shopping list + standing pantry ledger. Written by the **unauthenticated** `/api/kitchen-list` route (the caterer has no member account) — the only public write surface in the app; the route hardcodes this key, validates shape (`groups` + `pantry` arrays) and caps the payload at 200 KB, so the blast radius is this row alone. Not admin-editable UI; the page is the editor. Festival-scoped — see [catering-kitchen.md](catering-kitchen.md).
