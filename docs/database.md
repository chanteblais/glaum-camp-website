...
| `063_member_suspension.sql` | **Member suspension.** Adds `suspended_at` / `suspended_by` / `suspension_note` to `members`. A member (self-serve) or an admin can suspend attendance: releases the member's group + shift commitments and blocks new ones (`lib/suspension.ts`, gated in `/api/signup`, `/api/shift-signups`, `/api/groups/membership`, `/api/groups/[id]/join`) while keeping full read access and their role/status intact. Suspended members are excluded from Overview participation counts (shown in their own box) and attunement nudges. Additive + idempotent, non-destructive. |
