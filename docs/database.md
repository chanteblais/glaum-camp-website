...
- `catering_kitchen_state` / `catering_kitchen_state_test` — **Retired 2026-08-26** (kitchen board extracted to the All Hands repo; state copied to the All Hands Supabase at cutover). Nothing in this app reads or writes them anymore — `/api/kitchen-list` was deleted with the board. Any remaining rows are orphaned snapshots, safe to delete by hand.
