...
| `show_on_dashboard` | BOOL | default **false** (migration `065`). Opt-in: when on, the list appears as a row on the home "Bring Something" dashboard widget. Toggled in the list create/edit form on `/participate`. Distinct from `visible` (which governs the participate board) |
