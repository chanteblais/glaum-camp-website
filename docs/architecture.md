...
| `/api/apply/file` | POST / GET | POST uploads an attachment for an admin-added **File upload** application field (multipart `file` → **private** `application-files` bucket under `{userId}/`, migration `072`; returns `{ url, name }` where `url` is a GET href to this route, not a storage URL). Type allow-list + 10 MB cap + per-user quota (30 files / 100 MB). GET `?path=` is the only read path: **owner-or-admin**, then a 302 to a 60s signed URL (`lib/application-files.ts` rewrites pre-`072` public URLs stored in old answers to this route at render time). Upload gate is **any signed-in Clerk user** (not just applicants) and there's no cleanup path — see the gate note below the tables. |
