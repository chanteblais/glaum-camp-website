...
| `/api/resources/lists` | POST | **Member-owned resources (2026-07-08).** Create a list: `{ title, description, show_on_dashboard? }` (goes live immediately; `show_on_dashboard` default false — opt-in to the home widget, migration `065`). **Approved, non-suspended members.** |
