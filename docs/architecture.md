...
Auth is handled by **Clerk v7**. The Clerk middleware runs on all routes via `proxy.ts` (Next 16's rename of the `middleware.ts` convention).
...
- **Server-rendered section data:** when a page's client components need API data on mount, extract the GET's assembly into a lib function and have the page fetch it server-side, passing it as an `initial*` prop (the component skips its mount fetch when the prop is present); the API route keeps serving the client's refresh path. Precedents: `lib/inbox.ts` (`/messages`), `lib/participate-data.ts` + `lib/resources.ts` `getMemberResourceView` (`/participate` — `SignupSection`, `GroupCommitments`, `ResourceCommitments`), `lib/admin-program-data.ts` (`/admin/program` — `ScheduleManager`, `LeadUpGatheringsManager`; also the assembly behind the schedule/rosters/shift-types/lead-up-events admin GETs), `lib/lead-up.ts` (`/schedule` — `LeadUpGatherings`; also `GET /api/lead-up-events`). **Caveat — cross-tab registries:** if the `initial*` data is *authored on a different page* (e.g. shift types: authored on Configure, consumed by `ScheduleManager` on Program), the server-rendered prop can be stale — Next's router cache prefetches the consuming page before the edit happens. Such components must background-refetch on mount (and ideally again when the picker/modal opens) instead of skipping the fetch; the prop stays only the first paint.
...
|---|---|---|
| `/api/kitchen-list` | GET/PUT/POST | **Unauthenticated** (deliberate): read/write the catering kitchen board's shared state (`page_content.catering_kitchen_state`). Backs `public/kitchen.html`, used by the caterer, who has no member account. Key comes from a closed `scope` allow-list (`live`, or `test` for a scratch board — never caller-supplied), payload shape-validated (`days` **or** legacy `groups`, plus `pantry`) and capped at 200 KB; POST exists only so the page's leave-flush `sendBeacon` works. Last-write-wins, ~10 s poll. See [catering-kitchen.md](catering-kitchen.md). |
...
The two `event-reminders` entries share one route (`lib/event-reminders.ts` `collectEventReminders` gathers RSVP'd gatherings + held shift occurrences for the target camp-local date). Dedupe is the `event_reminders_sent` ledger (`clerk_user_id` × `target_date` × `phase`), so a re-fire or overlap never double-sends; opt-out is `notification_preferences.email_event_reminders`; suspended/non-approved members are skipped; sends spaced 600ms; `maxDuration = 60`; **admin** GET dry-runs by default (`?send=1` to send). With no `?phase` the route runs **both** phases (a safe fallback if the query string is dropped). Camp timezone (`America/Vancouver`) is currently hardcoded — see generalizability log. **Vercel plan note:** this brings the project to **3 cron entries**; if the Vercel plan caps cron jobs below that, consolidate to a single no-`phase` daily entry (both reminders fire at one time) or upgrade. |
