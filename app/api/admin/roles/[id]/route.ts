...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabaseAdmin
    .from('roles')
    .update({ name, description, capacity, sort_order, department_id, purpose, responsibilities_before, responsibilities_during, ideal_for, commitment, commitment_period, requires_approval })
    .eq('id', params.id)
    .select()
    .single()
