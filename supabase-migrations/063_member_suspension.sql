...
-- their group + shift commitments are released and the join endpoints refuse
-- new ones until the suspension is lifted, but they keep full read access to
-- the site and their status stays 'approved'. Suspension is deliberately
-- orthogonal to members.status so every existing "approved member" gate keeps
-- working unchanged.
--
