// Reusable asset library — curated, themeable art that admin features pick from
// (starting with Distinction medals; widen `AssetCategory` as it grows).
//
// SaaS direction (see docs/generalizability-log.md): there are two libraries.
//   • BUILT-IN — art shipped in the repo under public/asset-library/, shared by
//     every community. Defined here in code. No DB, no per-tenant state.
//   • UPLOAD — art a single community uploads for itself. Post–What If this moves
//     to a tenant-scoped DB table (asset_library, with community_id) and is merged
//     into the same picker behind the AssetLibraryItem shape — so consumers never
//     learn where an asset came from.
// Until uploads exist, the built-ins below ARE the whole library.
//
// NOTE: the "Elder Tree" emblem originated as Glåüm-specific branding. Shipping
// it as a built-in is a single-tenant expedient for the dogfood; in multi-tenant
// it should be a tenant UPLOAD, with built-ins reserved for generic/global art.
// Labels are kept generic so the library reads cleanly for any community.

export type AssetKind = 'raster' | 'svg'

// Image groups in the library. A picker shows its primary category first but lets
// the user browse the others. Add categories (badges, schedule icons, …) as
// features adopt the library.
export type AssetCategory = 'distinction' | 'icon'

// Display order + labels for category tabs. NOTE: keep labels generic (no
// community-specific wording) — the built-in library is meant to be global.
export const ASSET_CATEGORIES: { value: AssetCategory; label: string }[] = [
  { value: 'distinction', label: 'Distinctions' },
  { value: 'icon', label: 'Icons' },
]

// Categories ordered with `primary` first, so a picker surfaces the relevant
// type up front while still exposing the rest.
export function orderedCategories(primary: AssetCategory): { value: AssetCategory; label: string }[] {
  return [...ASSET_CATEGORIES].sort((a, b) =>
    a.value === primary ? -1 : b.value === primary ? 1 : 0)
}

export type AssetLibraryItem = {
  id: string
  label: string
  /** Public path (built-in) or storage URL (upload). Goes straight into a rule's `image`. */
  src: string
  kind: AssetKind
  category: AssetCategory
  /** Built-in (shipped in repo) vs uploaded by a community. */
  source: 'builtin' | 'upload'
  /** Optional search/filter tags for the picker. */
  tags?: string[]
}

// The built-in library. Append entries as curated art is added. Keep `label`s
// generic (describe the art, not a specific community/honour).
export const BUILTIN_ASSETS: AssetLibraryItem[] = [
  {
    id: 'elder-tree',
    label: 'Elder Tree',
    src: '/asset-library/distinctions/elder-tree.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['tree', 'laurel', 'elder', 'hands', 'nature'],
  },
  {
    id: 'flame-in-hands',
    label: 'Flame in Hands',
    src: '/asset-library/distinctions/flame-in-hands.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['flame', 'hands', 'fire', 'spark', 'founding', 'first'],
  },
  {
    id: 'seedling-in-hands',
    label: 'Seedling in Hands',
    src: '/asset-library/distinctions/seedling-in-hands.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['seedling', 'sprout', 'hands', 'planting', 'growth', 'beginning'],
  },
  {
    id: 'campfire',
    label: 'Campfire',
    src: '/asset-library/distinctions/campfire.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['campfire', 'fire', 'logs', 'hearth', 'gathering', 'wreath'],
  },
  {
    id: 'sunrise',
    label: 'Sunrise',
    src: '/asset-library/distinctions/sunrise.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['sunrise', 'sun', 'dawn', 'rays', 'horizon', 'beginning'],
  },
  {
    id: 'wreathed-key',
    label: 'Wreathed Key',
    src: '/asset-library/distinctions/wreathed-key.webp',
    kind: 'raster',
    category: 'distinction',
    source: 'builtin',
    tags: ['key', 'wreath', 'unlock', 'opening', 'founding'],
  },
  {
    id: 'ornate-urn',
    label: 'Ornate Urn',
    src: '/asset-library/icons/ornate-urn.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['urn', 'vase', 'ornament', 'fleur-de-lis', 'flourish', 'pedestal', 'decorative'],
  },
  {
    id: 'raised-hand',
    label: 'Raised Hand',
    src: '/asset-library/icons/raised-hand.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['hand', 'palm', 'raised', 'open-hand', 'gesture'],
  },
  {
    id: 'classical-temple',
    label: 'Classical Temple',
    src: '/asset-library/icons/classical-temple.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['temple', 'columns', 'pillars', 'pediment', 'building', 'classical', 'institution'],
  },
  {
    id: 'flaming-chalice',
    label: 'Flaming Chalice',
    src: '/asset-library/icons/flaming-chalice.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['flame', 'fire', 'chalice', 'goblet', 'brazier', 'lotus', 'pedestal'],
  },
  {
    id: 'covered-wagon',
    label: 'Covered Wagon',
    src: '/asset-library/icons/covered-wagon.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['wagon', 'cart', 'wheels', 'canopy', 'pioneer', 'transport', 'travel'],
  },
  {
    id: 'fruit-bowl',
    label: 'Bowl of Fruit',
    src: '/asset-library/icons/fruit-bowl.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['bowl', 'fruit', 'apple', 'bread', 'grapes', 'harvest', 'food', 'pedestal'],
  },
  {
    id: 'lantern',
    label: 'Lantern',
    src: '/asset-library/icons/lantern.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['lantern', 'lamp', 'flame', 'fire', 'light', 'glow', 'guide'],
  },
  {
    id: 'tasselled-cushion',
    label: 'Tasselled Cushion',
    src: '/asset-library/icons/tasselled-cushion.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['cushion', 'pillow', 'tassel', 'velvet', 'comfort', 'decor', 'lounge'],
  },
  {
    id: 'pitched-tent',
    label: 'Pitched Tent',
    src: '/asset-library/icons/pitched-tent.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['tent', 'camp', 'shelter', 'canvas', 'pitch', 'setup', 'glow'],
  },
  {
    id: 'mallet-and-stake',
    label: 'Mallet & Stake',
    src: '/asset-library/icons/mallet-and-stake.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['mallet', 'hammer', 'stake', 'peg', 'tools', 'build', 'teardown'],
  },

  {
    id: 'eye-in-triangle',
    label: 'Eye in Triangle',
    src: '/asset-library/icons/eye-in-triangle.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['eye', 'triangle', 'witness', 'attunement', 'rays', 'trinity', 'sight'],
  },
  {
    id: 'elder-tree',
    label: 'Elder Tree',
    src: '/asset-library/icons/elder-tree.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['tree', 'roots', 'canopy', 'elder', 'nature', 'growth'],
  },
  {
    id: 'crescent-moon',
    label: 'Crescent Moon',
    src: '/asset-library/icons/crescent-moon.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['moon', 'crescent', 'night', 'celestial', 'sky'],
  },
  {
    id: 'flame',
    label: 'Flame',
    src: '/asset-library/icons/flame.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['flame', 'fire', 'spark', 'light', 'ceremony'],
  },
  {
    id: 'star',
    label: 'Star',
    src: '/asset-library/icons/star.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['star', 'sparkle', 'celestial', 'four-point', 'night'],
  },
  {
    id: 'heart',
    label: 'Heart',
    src: '/asset-library/icons/heart.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['heart', 'love', 'care', 'social'],
  },
  {
    id: 'clipboard',
    label: 'Clipboard',
    src: '/asset-library/icons/clipboard.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['clipboard', 'checklist', 'tasks', 'planning', 'services'],
  },
  {
    id: 'gathering',
    label: 'Gathering',
    src: '/asset-library/icons/gathering.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['people', 'gathering', 'group', 'community', 'mixer'],
  },
  {
    id: 'teacup',
    label: 'Teacup',
    src: '/asset-library/icons/teacup.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['tea', 'teacup', 'steam', 'nourishment', 'drink', 'warmth'],
  },
  {
    id: 'hand-mirror',
    label: 'Hand Mirror',
    src: '/asset-library/icons/hand-mirror.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['mirror', 'reflection', 'attunement', 'vanity', 'ornate'],
  },
  {
    id: 'radiant-sun',
    label: 'Radiant Sun',
    src: '/asset-library/icons/radiant-sun.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['sun', 'rays', 'radiant', 'day', 'light'],
  },
  {
    id: 'shrimp',
    label: 'Shrimp',
    src: '/asset-library/icons/shrimp.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['shrimp', 'prawn', 'sponsor', 'sea', 'feast'],
  },
  {
    id: 'crescent-eye',
    label: 'Crescent Eye',
    src: '/asset-library/icons/salon-eye.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['eye', 'crescent', 'lashes', 'salon', 'rays', 'sight'],
  },
  {
    id: 'envelope',
    label: 'Sealed Envelope',
    src: '/asset-library/icons/envelope.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['envelope', 'letter', 'message', 'mail', 'seal', 'hand'],
  },
  {
    id: 'bell',
    label: 'Hand Bell',
    src: '/asset-library/icons/bell.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['bell', 'announcement', 'ring', 'call', 'notice'],
  },
  {
    id: 'scroll',
    label: 'Scroll',
    src: '/asset-library/icons/scroll.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['scroll', 'registry', 'rules', 'document', 'parchment'],
  },
  {
    id: 'quill',
    label: 'Quill',
    src: '/asset-library/icons/quill.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['quill', 'feather', 'pen', 'writing', 'application'],
  },
  {
    id: 'candle',
    label: 'Candle',
    src: '/asset-library/icons/candle.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['candle', 'flame', 'ceremony', 'light', 'vigil'],
  },
  {
    id: 'paintbrush',
    label: 'Paintbrush',
    src: '/asset-library/icons/paintbrush.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['paintbrush', 'art', 'decor', 'paint', 'craft'],
  },
  {
    id: 'cooking-pot',
    label: 'Cooking Pot',
    src: '/asset-library/icons/cooking-pot.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['pot', 'cooking', 'kitchen', 'meal', 'nourishment', 'steam'],
  },
  {
    id: 'broom',
    label: 'Broom',
    src: '/asset-library/icons/broom.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['broom', 'sweep', 'cleanup', 'teardown', 'tidy'],
  },
  {
    id: 'signpost',
    label: 'Signpost',
    src: '/asset-library/icons/signpost.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['signpost', 'wayfinding', 'directions', 'orientation', 'arrows'],
  },
  {
    id: 'hourglass',
    label: 'Hourglass',
    src: '/asset-library/icons/hourglass.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['hourglass', 'time', 'schedule', 'sand', 'patience'],
  },
  {
    id: 'crystal-ball',
    label: 'Crystal Ball',
    src: '/asset-library/icons/crystal-ball.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['crystal', 'ball', 'orb', 'attunement', 'mystic', 'sparkle'],
  },
  {
    id: 'disco-ball',
    label: 'Disco Ball',
    src: '/asset-library/icons/disco-ball.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['disco', 'ball', 'party', 'dance', 'mirror', 'sparkle'],
  },
  {
    id: 'retro-microphone',
    label: 'Retro Microphone',
    src: '/asset-library/icons/retro-microphone.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['microphone', 'mic', 'voice', 'radio', 'broadcast', 'retro'],
  },
  {
    id: 'vintage-radio',
    label: 'Vintage Radio',
    src: '/asset-library/icons/vintage-radio.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['radio', 'receiver', 'dial', 'broadcast', 'retro', 'tune'],
  },
  {
    id: 'radio-tower',
    label: 'Radio Tower',
    src: '/asset-library/icons/radio-tower.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['tower', 'antenna', 'broadcast', 'signal', 'on-air', 'mast'],
  },
  {
    id: 'megaphone',
    label: 'Megaphone',
    src: '/asset-library/icons/megaphone.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['megaphone', 'announcement', 'broadcast', 'loud', 'call'],
  },
  {
    id: 'gramophone',
    label: 'Gramophone',
    src: '/asset-library/icons/gramophone.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['gramophone', 'music', 'horn', 'record', 'retro', 'sound'],
  },
  {
    id: 'pennant',
    label: 'Pennant',
    src: '/asset-library/icons/pennant.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['pennant', 'flag', 'milestone', 'marker', 'hand'],
  },
  {
    id: 'songbird',
    label: 'Songbird',
    src: '/asset-library/icons/songbird.webp',
    kind: 'raster',
    category: 'icon',
    source: 'builtin',
    tags: ['bird', 'songbird', 'lark', 'early', 'dawn', 'morning', 'song', 'arrival', 'setup'],
  },
]

/** Built-in assets usable in a given category. */
export function builtinAssets(category: AssetCategory): AssetLibraryItem[] {
  return BUILTIN_ASSETS.filter(a => a.category === category)
}
